# PROMPT FASE 5 — Documentos (Generación, Previsualización, Descarga)
...
Crear la vista de Documentos completa:
- **Listar** todos los documentos generados (con filtros por matter, estado, tipo)
- **Generar nuevo** documento con wizard de 3 pasos (matter → template → revisar → generar)
- **Previsualizar** PDF en el navegador (iframe o nueva pestaña)
- **Descargar** PDF directamente
- **Cambiar estado** (borrador → revisión → firmado → archivado)
- **Ver historial** de documentos por matter
...
dashboard/frontend/
├── js/
│   ├── app.js          (MODIFICAR — agregar loadDocumentos real)
│   └── documentos.js   (CREAR — toda la lógica de documentos)
└── css/
    └── kami.css        (AGREGAR al final — estilos de documentos)
```

---

## 🔧 PASO 1: Crear `js/documentos.js`
...
**Guardar en:** `~/ws-hermes-legal-pro/dashboard/frontend/js/documentos.js`

---

## 🔧 PASO 2: Modificar `js/app.js`

Buscar `loadDocumentos()` placeholder y reemplazar:
...
Agregar script de documentos antes de app.js:
...
### Test 1: Lista de documentos carga
```
Abrir http://localhost:8082 → Click en "Documentos"
Esperado: Lista con tarjetas, filtros, búsqueda
```

### Test 2: Wizard de generación
```
Click "+ Generar Documento" → Seleccionar matter → Siguiente
Seleccionar template → Siguiente → Revisar datos → Generar
Esperado: PDF generado y descargado
```

### Test 3: Previsualizar PDF
```
Click "👁️ Ver" en documento con PDF
Esperado: Abre PDF en nueva pestaña
```

### Test 4: Descargar PDF
```
Click "⬇️ Descargar" en documento
Esperado: Archivo se guarda en Downloads
```

### Test 5: Cambiar estado
```
Cambiar select de "Borrador" a "Generado" o "Firmado"
Esperado: Badge cambia de color
```

### Test 6: Filtros funcionan
```
Filtro "Borradores" → Solo muestra borradores
...
## ✅ CHECKLIST PARA PASAR A FASE 6

- [ ] documentos.js creado y funciona
- [ ] Lista muestra documentos reales
- [ ] Wizard de 3 pasos funciona
- [ ] Generar documento crea PDF real
- [ ] Previsualizar abre PDF
- [ ] Descargar guarda archivo
...
git commit -m "FASE 5: Documentos - wizard, generación, previsualización, descarga"
git push origin master
```

**Notificar:** "FASE 5 completada. Documentos funcionando al 100%. Listo para FASE 6 (Calendario)."

---

*Prompt FASE 5 — Diseñado para OpenCode Go*
